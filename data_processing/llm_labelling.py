...
conn = sqlite3.connect("../data/labelled/labelled_reddit_posts.db")
cursor = conn.cursor()
...
def store_in_db(conn, row, timestamp):
    cursor = conn.cursor()
    try:
        cursor.execute('''
            INSERT INTO labelled_reddit_posts (id, title, label)
            VALUES (?, ?, ?)
        ''', (
            row.id,
            row.title,
